...
/**
 * Servlet implementation class u_saveproduct
 */
@WebServlet("/u_saveproduct")
public class u_saveproduct extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public u_saveproduct() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet rsLogin12 = stRegister12
					.executeQuery("select * from user_product_info where Product_No='"+product_No+"'");
			if (rsLogin12.next()) {
			...
			ResultSet rs1;
			String str1 = "select * from user_product_info where Product_No='"+product_No+"'";
			...
			if (rs1.next()) {
				...
				ss.executeUpdate("update user_product_info set ProductName='"+productName+"', CompanyName='"+companyname+"', DescriptionName='"+Descriptionname+"', MFGDate='"+mfgdate+"',Price='"+price+"',ExpiryDate='"+Expirydate+"',Qty='"+newtotalqty+"' where Product_No='"+product_No+"'");
				...
				if (rs11.next()) {
					...
					st.executeUpdate("insert into user_product_info_bill(ProductName, CompanyName, DescriptionName, MFGDate,Price,ExpiryDate,Qty,product_No,Paybill,Email_ID,User_Mobile_No) values ('"
							+ productName
							+ "','"
							+ companyname
							+ "','"
							...
							+ qty
							+ "','"
							+ product_No + "','"+totalprice+"','"+emailid+"','"+User_Mobile_No+"')");
				...
				int newqty=Integer.parseInt(qty);
				int newtotalqty=oqty+newqty;
				st.executeUpdate("insert into user_product_info_bill(ProductName, CompanyName, DescriptionName, MFGDate,Price,ExpiryDate,Qty,Product_No) values ('"
						+ productName
						+ "','"
						+ companyname
						+ "','"
						+ Descriptionname
						+ "','" + mfgdate + "','"+price+"','"+Expirydate+"','"+newtotalqty+"','"+product_No+"')");
				Statement stt = (Statement) con.createStatement();
				ResultSet rs11;
				String str = "select * from tbluser_amount where Email_ID='"
						+ emailid + "'";
			...
			int noofnode=4;
			String datainfo=productName+companyname+Descriptionname+mfgdate+price+Expirydate+qty+product_No+totalprice+emailid+User_Mobile_No;
